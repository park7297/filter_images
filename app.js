...
// json 객체 데이터 배열(filterItems)을 forEach 메서드로 순회 - 배열 요소 각각을 분리 - 분리된 각각의 데이터는 item 파라미터에 전달
filterItems.forEach((item) => {
  // 각각의 의미지 요소 생성 - DOM(Document Object Model : html 문서의 요소들) 생성
  const itemElmt = `
...
function showItems(thisItem) {
  ...
  if (thisCategory === "all") {
    // 클릭한 요소의 속성값이 all일 때
    showAndHide(images, true); // images array add show remove
  }
  ...
  showAndHide(thisImage, true); // images array add show remove
}
...
// 첫번째 파라미터 : 함수 - 실행할 코드, 두번째 파라미터 : 숫자 - 시간(밀리초) = 단위 시간 이후 함수 실행
// setTimeout(function () {
